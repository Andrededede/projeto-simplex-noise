...
def compare_images(img1_path: str, img2_path: str) -> Dict:
    ...
    Args:
        ...
        img2_path: Caminho para segunda imagem (Java)
    ...
    if (w1, h1) != (w2, h2):
        return {
            'identical': False,
            'error': f'Dimensões diferentes: C({w1}x{h1}) vs Java({w2}x{h2})'
        }

    # Verificar identidade
    identical = np.array_equal(img1, img2)

    # Calcular diferenças COM SINAL (C - Java)
    ...
    # Diferença média com sinal (indica bias: positivo = C maior, negativo = Java maior)
...
def plot_difference_heatmap(img1_path: str, img2_path: str, output_path: str):
    ...
    # Subplot 2: Imagem Java
    axes[1].imshow(img2, cmap='gray', vmin=0, vmax=255)
    axes[1].set_title('Implementação Java', fontsize=12, fontweight='bold')
    axes[1].axis('off')
...
def print_comparison_result(filename: str, result: Dict):
    ...
    print(
        f"      └─ Valor C: {result['max_diff_c']}, Valor Java: {result['max_diff_py']}")
    ...
    print(f"   Diferença nas médias (C - Java): {result['diff_means']:+.2f}")
    print(
        f"   Diferença nos desvios padrão (C - Java): {result['diff_stds']:+.2f}")


def main():
    """Função principal que executa os testes de comparação."""
    # Definir caminhos (agora test está dentro de python_implementation)
    c_filtered_dir = "comparar/imagemC"
    python_filtered_dir = "comparar/imagemJava"
    ...
    print(f"\n{'='*70}")
    print(f"COMPARAÇÃO DE IMAGENS: Implementação C vs Java")
    print(f"{'='*70}")

    # Criar diretório para armazenar os heatmaps
    test_imgs_dir = "comparar/imgs_tests"
    ...
    for filename in sorted(c_files):
        ...
        if not os.path.exists(python_path):
            print(f"\n[X] Arquivo {filename} não encontrado na versão Java")
            continue
